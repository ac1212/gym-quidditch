...
class QuidditchSeekerEnv(gym.Env):
    ...
    def step(self, action):
        ...
        if snitch_x[0]<-self.pitch_size/2: # hit left wall
            ...
            snitch_v[0] *= -1*self.elasticity
        if snitch_x[0]>self.pitch_size/2: # hit right wall
            snitch_x[0] = self.pitch_size/2
            snitch_v[0] *= -1*self.elasticity
        if snitch_x[1]<-self.pitch_size/2: # hit bottom wall
            snitch_x[1] = -self.pitch_size/2
            snitch_v[1] *= -1*self.elasticity
        if snitch_x[1]>self.pitch_size/2: # hit top wall
            snitch_x[1] = self.pitch_size/2
            snitch_v[1] *= -1*self.elasticity
